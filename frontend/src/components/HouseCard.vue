...
const totalPrice = computed(() =>
  props.data.square * props.data.price
)
</script>
...
<template>
  <router-link :to="link" class="bg-white shadow rd overflow-hidden
    ...
    <div class="p-4">
      <h3 class="font-light text-sm m-0">{{ props.data.name }}</h3>
      <p class="m-0 text-lg">
        <span class="font-bold text-red-5">¥{{ totalPrice }}</span>
        <span class="text-neutral"> / </span>
        <span class="text-neutral font-light">{{ props.data.square }} m<sup>2</sup></span>
      </p>
    </div>
  </router-link>
</template>
